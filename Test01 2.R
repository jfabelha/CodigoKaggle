...
# adding min and max ROC AUC to the center of the plot
minauc2<-min(round(auc2, digits = 5))
maxauc2<-max(round(auc2, digits = 5))
...
arv3<-rpart(newtrain$TARGET ~ .,data=newtrain, method="class")
Pred_arv3<-predict(arv3, newtest, type="prob")
submission3 <- data.frame(ID=test.id, TARGET=Pred_arv3)
cat("saving the submission file\n")
write.csv(submission3, "submissionRFarv.csv", row.names = F)


nb2<- naiveBayes(newtrain$TARGET ~ .,data=newtrain, method="class")
Pred_nb2<-predict(nb2, newtest, type="class")
...
lm2 <- glm(newtrain$TARGET ~ .,data=newtrain)
Pred_lm<-predict(lm2, newtest, type="response")
submission2 <- data.frame(ID=test.id, TARGET=Pred_lm)
cat("saving the submission file\n")
write.csv(submission2, "submissionRFglm.csv", row.names = F)
...
#redes neuronais
library(nnet)
nn <- nnet(newtrain$TARGET ~ .,data=newtrain, MaxNWts=2591,hidden=c(5,3),size=10 , maxit = 1000,  linout = T)
Pred_nn<-predict(nn, amostra_treino, type="prob")
...
fit <- randomForest(as.factor(amostra_treino$train.y)~., data=amostra_treino, importance=TRUE, ntree=2000)
